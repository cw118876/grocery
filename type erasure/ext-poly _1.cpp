 ...
 public:
  ...
  class ShapeDetail: public ShapeConcept {
    public:
      ...
      void do_transfer(Vector2D& v) final {
        transfer(value_, v);
      }
      void do_rotate(double const& d) final {
        rotate(value_, d);
      }
      void do_draw() final {
        draw(value_);
  ...
  explicit Shape(ShapeT shape): pimp_{std::make_unique<ShapeDetail<ShapeT>>(std::move(shape))} {}
