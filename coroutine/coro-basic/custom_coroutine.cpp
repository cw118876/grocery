  ...
  class promise_type {
   public:
    ...
    struct final_awaiter {
      ...
      std::coroutine_handle<> await_supsend(std::coroutine_handle<promise_type>) noexcept;
...
inline std::coroutine_handle<> task::promise_type::final_awaiter::await_supsend(std::coroutine_handle<task::promise_type> h) noexcept {
...
inline task::awaiter::awaiter(std::coroutine_handle<promise_type> h) noexcept : coro_ {h} {
}
...
inline task::task(std::coroutine_handle<task::promise_type> h) noexcept: coro_{h} {}
...
struct destructor_guard {
  explicit destructor_guard(manual_lifetime<T>& obj): ptr_{std::addressof(obj)} {}
...
struct __g_state: __coroutine_state_with_promise<__g_promise_t> {
  __g_state(int &&x): _x(static_cast<int &&>(x)) {
    ...
    ::new ((void*) std::addressof(this->__promise_)) __g_promise_t(construct_from<__g_promise_t>(this->_x));
  }
...
task g(int x) {
  ...
  state->__temp1.construct_from([&]() -> decltype(auto) {
    return state->__promise_.initial_suspend();
  });

  if (!state->__temp1.get().await_ready()) {
    state->__temp1.get().await_suspend(std::coroutine_handle<__g_promise_t>::from_promise(state->__promise_));
    state.release();
  } else {
